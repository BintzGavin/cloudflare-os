...
async function compareHere(
  ...
  const mismatch = statusMismatch(status, oracleStatus(oracle), fields);
  if (mismatch.length > 0) {
    return { at: fen, what: `status.${mismatch.join(",")}`, gadget: status, oracle: oracleStatus(oracle) };
  }
  return null;
}
      ...
      await verifier.check("starts-from-the-standard-position", async () => {
        ...
        return {
          pass: started === DEFAULT_POSITION && read === DEFAULT_POSITION &&
            statusMismatch(status, oracleStatus(new Chess()), BASE_STATUS).length === 0 &&
    ...
    verify: async verifier => {
      ...
      await verifier.check("exports-pgn-the-oracle-replays-to-the-same-position", async () => {
        ...
        for (const [name, setUp] of Object.entries(cases)) {
          ...
          if (!sameFen(replay.fen(), oracle.fen())) {
            failures[name] = { exported, replay: replay.fen(), oracle: oracle.fen() };
          }
...
gameOver is true for checkmate or draw. Everything that already worked keeps working.`,
    verify: async verifier => {
      await verifier.check("detects-threefold-repetition-and-the-fifty-move-rule", async () => {
        ...
        const expectedThree = oracleStatus(oracle);
        return {
          pass: afterTwo !== undefined && afterThree !== undefined &&
            ...
            statusMismatch(afterFifty, oracleStatus(fiftyOracle), DRAW_STATUS).length === 0 &&
      ...
      await checkCurated(verifier, "judges-material-and-terminal-positions-like-the-oracle", DRAW_STATUS, {
        ...
        sameColourBishops: "8/8/8/8/8/2b1k3/8/4K2B w - - 0 1",
