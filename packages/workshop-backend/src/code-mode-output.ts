...
import {
  MAX_CHAT_ATTACHMENTS_PER_MESSAGE, validateCodeImageAttachment,
  ...
  for (let [index, block] of value.slice(0, MAX_CHAT_ATTACHMENTS_PER_MESSAGE).entries()) {
    try {
      if (typeof block?.data !== "string" || typeof block.mimeType !== "string") {
        throw new Error("Image is missing its data or MIME type.");
      }
      ...
      // This function owns the size budget above; the validator checks only format and signature.
      images.push({mimeType: validateCodeImageAttachment(block.mimeType, content), content});
